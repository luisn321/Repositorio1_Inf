    ...
    public class ContractionRepository : IContractionRepository
    {
            ...
            try
            {
                var result = await _db.ExecuteNonQueryAsync(
                    ...
                    new Dictionary<string, object>
                    {
                        ...
                        { "monto", (object?)contraction.MontoPropuesto ?? DBNull.Value },
                        ...
                        { "monto_pagado", (object?)contraction.MontoPagado ?? DBNull.Value },
                        { "payment_intent_id", string.IsNullOrEmpty(contraction.PaymentIntentId) ? (object)DBNull.Value : contraction.PaymentIntentId },
                        { "clabe_tecnico", string.IsNullOrEmpty(contraction.ClabeTecnico) ? (object)DBNull.Value : contraction.ClabeTecnico },
        ...
        public async Task<bool> AssignTechnicianAsync(int contractionId, int technicianId, double? montoPropuesto)
        {
            try
            {
                ...
                {
                    _logger.LogWarning($"No se actualizó ningún registro. ¿Existe contractionId {contractionId}?");
                }
        ...
        private ContractionModel MapToContractionModel(Dictionary<string, object> data)
        {
            return new ContractionModel
            {
                ...
                NombreServicio = data.ContainsKey("nombre_servicio") && data["nombre_servicio"] != DBNull.Value ? (string)data["nombre_servicio"] : null,
                ...
                MontoPropuesto = data.ContainsKey("monto_propuesto") && data["monto_propuesto"] != DBNull.Value ? Convert.ToDecimal(data["monto_propuesto"]) : null,
                ...
                PaymentIntentId = data.ContainsKey("payment_intent_id") && data["payment_intent_id"] != DBNull.Value ? (string)data["payment_intent_id"] : null,
                ClabeTecnico = data.ContainsKey("clabe_tecnico") && data["clabe_tecnico"] != DBNull.Value ? (string)data["clabe_tecnico"] : null
            };
        }
    }
}
